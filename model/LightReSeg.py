...
class LightReSeg(nn.Module):
    def __init__(self, nclass, in_channels):
        ...
        # Encode
        self.conv_encode1 = contracting_block(in_channels=in_channels, out_channels=16)
        ...
        self.final_layer = final_block(16, nclass)

        self.to_patch_embedding = nn.Sequential(
            Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=1, p2=1),
            nn.Linear(128, 128))
        self.recover =nn.Sequential(
            Rearrange(' b (h w) (p1 p2 c) -> b c (h p1) (w p2)', p1=1, p2=1,h=16),) #Change the value of h based on the size of the input image
...
if __name__ == "__main__":
    ...
    net = LightReSeg(nclass=2, in_channels=1)
    out = net(rgb)
    print(out[0].shape)