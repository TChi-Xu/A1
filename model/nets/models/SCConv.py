...
class CRU(nn.Module):
    def __init__(self, op_channel: int, alpha: float = 1 / 2, squeeze_radio: int = 2, group_size: int = 2,
        ...
        self.PWC1 = nn.Conv2d(up_channel // squeeze_radio, up_channel, kernel_size=1, bias=False)  # 创建卷积层

        # 下层特征转换
        self.PWC2 = nn.Conv2d(low_channel // squeeze_radio, low_channel - low_channel // squeeze_radio, kernel_size=1,
                              bias=False)  # 创建卷积层
...
class SCConv(nn.Module):
    def __init__(self, in_spatial: int, in_channels: int, group_num: int = 16, gate_treshold: float = 0.5, alpha: float = 1 / 2,
                 squeeze_radio: int = 2, group_size: int = 2, group_kernel_size: int = 3):
        super().__init__()  # 调用父类构造函数

        self.im_Spatial = nn.Sequential(nn.Conv2d(in_spatial, 1, kernel_size=1),
                                        nn.Sigmoid()
                                        )

        self.im_channels = nn.Sequential(nn.Conv2d(in_channels, 1, kernel_size=1),
                                         nn.Sigmoid()
                                         )

        self.SRU = SRU(in_spatial, group_num=group_num, gate_treshold=gate_treshold)  # 创建 SRU 层
        self.CRU = CRU(in_channels, alpha=alpha, squeeze_radio=squeeze_radio, group_size=group_size,
                       group_kernel_size=group_kernel_size)  # 创建 CRU 层

    def forward(self, x, y):
        z_spatial = x+y
        z_channels = torch.cat((x, y), dim=1)

        im_spatial = self.im_Spatial(z_spatial)
        im_channel = self.im_channels(z_channels)

        spatial_1 = torch.zeros_like(im_spatial)
        spatial_2 = torch.ones_like(im_spatial)

        channels_1 = torch.zeros_like(im_channel)
        channels_2 = torch.ones_like(im_channel)

        spatial = torch.where(im_spatial >= 0.5, spatial_1, spatial_2)
        channels = torch.where(im_channel >= 0.8, channels_1, channels_2)

        spatial = spatial * z_spatial
        channels = channels * z_channels
        #print(spatial.shape)
        #print(channels.shape)

        spatial = self.SRU(spatial)  # 应用 SRU 层
        channel = self.CRU(channels)  # 应用 CRU 层
        return spatial+channel


if __name__ == '__main__':
    x = torch.randn(1, 64, 64, 64)
    y = torch.randn(1, 64, 64, 64) # 创建随机输入张量
    model = SCConv(64, 128)  # 创建 ScConv 模型
