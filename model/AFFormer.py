...
class AFFormer(nn.Module):
    def __init__(self, nclasses, img_size=224, num_stages=4, num_path=[4, 4, 4, 4], num_layers=[1, 1, 1, 1], embed_dims=[64, 128, 256, 512],
        mlp_ratios=[8, 8, 4, 4], num_heads=[8, 8, 8, 8],  drop_path_rate=0.0,  in_chans=3, strides=[4, 2, 2, 2],
        pretrained=None, init_cfg=None,
    ):
        super(AFFormer, self).__init__()
    ...
    def forward(self, x):
        ...
        for idx in range(self.num_stages):
            ...
            print(att_inputs[0].shape)
            #print(x.shape)
            x = self.mhca_stages[idx](att_inputs, idx)
            features.append(x)

        x = features[-1] + features[-2]
        x = self.up1(x)
...
@register_model
def Afformer_base(nclass, in_channels):

    return AFFormer( nclasses=nclass,
...
@register_model
def Afformer_small(nclass, in_channels):
...
@register_model
def Afformer_tiny(nclass, in_channels):
