...
class Trainer(object):
    def __init__(self, args):
        ...
        self.model = get_segmentation_Model(name=args.model, nclass=cfg.TRAIN.CLASSES).to(self.device)  # GCtx_UNet

        # create network  #DeepLabV3, LRNNet、PP_LiteSeg、FasterNet、RepViT、AFFormer、LightReSeg
        #self.model = get_segmentation_Model(name=args.model, nclass=cfg.TRAIN.CLASSES,in_channels=cfg.TRAIN.IN_CHANNELS_1).to(self.device)
        #self.model = get_segmentation_Model(name=args.model, in_channels=cfg.TRAIN.IN_CHANNELS_1, num_classes=cfg.TRAIN.CLASSES).to(self.device) # PSPNet
        #self.model = get_segmentation_Model(name=args.model, num_classes=cfg.TRAIN.CLASSES, in_channels=cfg.TRAIN.IN_CHANNELS_1).to(self.device)  # CCNet
        #self.model = get_segmentation_Model(name=args.model, nclass=cfg.TRAIN.CLASSES, in_channels=cfg.TRAIN.IN_CHANNELS_1).to(self.device)  # CCNet
        ...
        # create criterion
        self.criterion = get_segmentation_loss(args.model, use_ohem=cfg.LOSS.USE_OHEM, aux=cfg.LOSS.AUX,
