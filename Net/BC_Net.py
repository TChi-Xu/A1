...
class BC_Net512(nn.Module):
    ...
    def __init__(self, block, layers, dilation=1, zero_init_residual=False,
        ...
        self.top_256 = nn.Sequential(
            nn.Conv2d(3, self.input_channels, kernel_size=5, stride=1, padding=2,
                      bias=False),
            norm_layer(self.input_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.input_channels, self.input_channels, kernel_size=3, stride=1, padding=1,
        ...
        # 256x256x128
        self.layer_256 = self._make_layer(block, 64, layers[0])
        # 256x256x128
        self.Branch_Attention_256 = Branch_Attention(128)
        # 128x128x256
    ...
    def forward(self, x):
        ...
        print(layer_512.shape)
        top_256 = self.top_256(input3)  # 256x256x64
        layer_256 = self.layer_256(top_256)  # 256x256x256
        print(layer_256.shape)

        Branch_Attention_256 = self.Branch_Attention_256(layer_256, layer_512)

        layer256_128 = self.layer256_128(Branch_Attention_256)

        top_128 = self.top_128(input4)
...
def _bc_net(block, layers):
    model = BC_Net512(block, layers)
    return model
...
if __name__ == '__main__':
    ...
    # net = bc_net().to(device)
    # # for i in net.modules():
    # #     print('+' * 20)
    # #     print(i)
    # #     print('+' * 20)
    # net = list(model.children())
    # # # print(len(net))
    # # # print(net[13])
    # print(type(net))
    # print(len(net))
