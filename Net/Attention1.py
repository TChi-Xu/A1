...
class Attention(nn.Module):
    ...
    def forward(self, x1, y1, z1):
        ...
        z3 = F.interpolate(z2, y1.size()[2:], mode='bilinear', align_corners=True)
        z3 = ((b < self.threshold2).float() * z3).cuda()
        y3 = ((b >= self.threshold2).float() * y2).cuda()
        y3 = y3 + z3

        y3 = F.interpolate(y3, x1.size()[2:], mode='bilinear', align_corners=True)

        y = ((a < self.threshold1).float() * y3).cuda()
